...
use nextid_sdk::{
    proof_service::{Endpoint, Action, Platform, ProofProcedure},
    util::{crypto::Secp256k1KeyPair, hex_encode, base64_encode},
    types::Result,
};
...
async fn main() -> Result<()> {
    ...
    std::io::stdin().read_line(&mut twitter_username).expect("Failed to read line");
    ...
    std::io::stdin().read_line(&mut avatar_pubkey).expect("Failed to read line");
    if avatar_pubkey.trim().len() == 0 {
        ...
        println!("Secret key: 0x{}", hex_encode(&avatar.sk.as_ref().unwrap().serialize()));

    } else {
        avatar = Secp256k1KeyPair::from_pk_hex(avatar_pubkey.trim())?;
    }
    avatar_pubkey = format!("0x{}", hex_encode(&avatar.pk.serialize_compressed())).trim().to_string();
    ...
    if procedure.avatar.has_sk() {
        let personal_sign = procedure.avatar.personal_sign(procedure.sign_payload.as_ref().unwrap())?;
        ...
        print!("{}", post_content);
        println!("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=\n");
        println!("Remember to replace '%SIG_BASE64%' to base64-ed signature just created by user.\n");
    }
    ...
    std::io::stdin().read_line(&mut tweet_status_id).expect("Failed to read line");
    procedure.submit(tweet_status_id.trim().to_string(), None, None).await?;
